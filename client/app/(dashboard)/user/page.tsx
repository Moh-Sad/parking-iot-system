...
export default function UserDashboardPage() {
  ...
  // Tick the elapsed timer every second when a session is active
  ...
  const elapsedSeconds = session
    ? Math.max(0, Math.floor((Date.now() - new Date(session.startTime).getTime()) / 1000)) + 0 * tick
    : 0;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            aria-label="Refresh"
            className="px-3 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:bg-muted/30 transition-colors"
          >
            <RefreshCw size={14} className={isRefreshing ? "animate-spin" : ""} />
          </button>
          <Link
            href="/user/finances"
            className="px-4 py-2 rounded-lg border border-border text-sm font-semibold text-foreground hover:bg-muted/30 transition-colors"
          >
            Top Up Wallet
          </Link>
        </div>
                ...
                <div className="grid grid-cols-2 gap-x-10 gap-y-4 w-full">
                  ...
                  <div className="flex items-end">
                    <button
                      ...
                      disabled={stopping}
                      className="flex items-center gap-2 px-4 py-2 rounded-lg border border-border text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors disabled:opacity-60"
                    >
            ...
            <>
              ...
              <div className="flex-1 border-t border-border/50 pt-4 flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-foreground">{formatMoney(wallet.monthlySpendCents, wallet.currency)}</span>
                </div>
              </div>
